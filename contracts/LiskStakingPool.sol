...
interface LiskTokenC {
    function balanceOf(address account) external view returns (uint256);
    function transfer(address recipient, uint256 amount) external returns (bool);
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);
}

contract LiskCollectionsC {
...
contract LiskStakingPool is LiskCollectionsC {
    struct Pool {
    ...
    function stake(uint256 poolId, uint256 amount) external {
        ...
        LiskTokenC(pool.stakingToken).transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 poolId, uint256 amount) external {
        ...
        LiskTokenC(pools[poolId].stakingToken).transfer(msg.sender, amount);
